...
public class TopicServiceImplTest {
    ...
    @Test
    public void shouldGetValid_IsTopicPresent() throws Exception {
        int topicId1 = DummyEntities.groupedTopic.id();
        int topicId2 = DummyEntities.unGroupedTopic.id();
        CompletionStage<List<String>> topicIds = CompletableFuture.supplyAsync(() -> {
            List<String> ids = new ArrayList();
        ...
        topicService.isTopicPresent(DummyEntities.groupedTopic.name()).handle((data, exception) -> {
            assertTrue(data);
            return null;
        }).toCompletableFuture().get();

        topicService.isTopicPresent("non-name").handle((data, exception) -> {
            assertFalse(data);
            return null;
        }).toCompletableFuture().get();
    }
