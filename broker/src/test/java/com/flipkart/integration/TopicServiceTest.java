...
import com.flipkart.vbroker.services.TopicTestUtils;
...
public class TopicServiceTest extends BaseClassForTests {
    ...
    public void shouldCreateTopic() throws Exception {
        CompletionStage<Topic> createStage = topicService.createTopic(TopicTestUtils.dummyTopic());
        Topic topic = createStage.toCompletableFuture().get();
        log.info("Id of created topic is " + topic.id());
        assertEquals(topic.name(), "topic1");
    ...
    public void topicIdShouldExistAfterCreation() throws Exception {
        short id = 101;
        CompletionStage<Topic> createStage = topicService.createTopicAdmin(id, TopicTestUtils.dummyTopic());
        Topic createdTopic = createStage.toCompletableFuture().get();
        assertEquals(createdTopic.name(), "topic1");
    ...
    public void topicNameShouldExistAfterCreation() throws Exception {
        ...
        CompletionStage<Boolean> existsStage1 = topicService.isTopicPresent("topic1");
    ...
    public void shouldGetTopicPartition_AfterCreateTopic() throws Exception {
        short topicId = 101;
        short partitionId = 1;
        CompletionStage<Topic> createStage = topicService.createTopicAdmin(topicId, TopicTestUtils.dummyTopic());
        Topic topic = createStage.toCompletableFuture().get();
